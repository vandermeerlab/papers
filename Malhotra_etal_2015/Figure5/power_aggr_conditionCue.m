...
for iTT = 1:length(trial_types)
    ...
    % normalize
    baseline = [-0.5 0];
    t_idx = nearest(tvec,baseline);
    lg_baseline = nanmean(lg_pow(:,t_idx(1):t_idx(2)),2); lg_baseline = repmat(lg_baseline,[1 trl_len]);
    hg_baseline = nanmean(hg_pow(:,t_idx(1):t_idx(2)),2); hg_baseline = repmat(hg_baseline,[1 trl_len]);
    lgz_baseline = nanmean(lg_powz(:,t_idx(1):t_idx(2)),2); lgz_baseline = repmat(lgz_baseline,[1 trl_len]);
    hgz_baseline = nanmean(hg_powz(:,t_idx(1):t_idx(2)),2); hgz_baseline = repmat(hgz_baseline,[1 trl_len]);
...
%stat_twin1 = [-0.75 -0.25];
%stat_twin2 = [0.25 0.75];
stat_twin1 = [-0.5 0];
stat_twin2 = [0 0.5];
...
for iTT = 1:length(trial_types)
    ...
    title('raw hanning 11-cycle');
    
    % baseline correct
    cfg = []; cfg.baseline = [-1 0]; cfg.baselinetype = 'relative';
    TFRb = ft_freqbaseline(cfg,TFR{iTT});
    
    hg_pwr = sq(nanmean(sq(TFRb.powspctrm(:,vStr_idx,F_idx,:)),2)); % average across frequencies
    lg_pwr = sq(nanmean(sq(TFRb.powspctrm(:,vStr_idx,F_idx2,:)),2));
...
fprintf('raw-bd Hanning lg pre 1p %.2f, 5p %.2f, ranksum p %3.2e\n',nanmean(lg_pwr_preb{1}),nanmean(lg_pwr_preb{2}),p_lg_pre);
