...
boxplot(dfb2$preCue.g50,dfb2$postCue.g50)
boxplot(dfb2$preCue.g80,dfb2$postCue.g80)
...
g50_pwr <- c(dfb2$preCue.g50,dfb2$postCue.g50)
g80_pwr <- c(dfb2$preCue.g80,dfb2$postCue.g80)
...
cueGamma_b2 <- data.frame(pp,subj,g50_pwr,g80_pwr,cueTime,preSpd,postSpd,xcoord,prevRew)

# is the mixed model better than the one without?
ml <- lm(g50_pwr ~ 1 + pp,data=cueGamma_b2)
m0 <- lmer(g50_pwr ~ (1|subj),data=cueGamma_b2,REML=FALSE)
m1 <- lmer(g50_pwr ~ (1|subj) + pp,data=cueGamma_b2,REML=FALSE)
...
m0 <- lmer(g50_pwr ~ (1|subj) + cueTime,data=cueGamma_b1,REML=FALSE)
m1 <- lmer(g50_pwr ~ (1|subj) + cueTime + pp,data=cueGamma_b1,REML=FALSE)

anova(m0,m1)

# --------------- gamma-80 -------------------------------------
ml <- lm(g80_pwr ~ 1 + pp,data=cueGamma_b2)
m0 <- lmer(g80_pwr ~ (1|subj),data=cueGamma_b2,REML=FALSE)
m1 <- lmer(g80_pwr ~ (1|subj) + pp,data=cueGamma_b2,REML=FALSE)
