...
# inspect
summary(dfb1$preNP.g50)
summary(dfb1$preNP.g80)
summary(dfb1$postNP.g50)
summary(dfb1$postNP.g80)

boxplot(dfb1$preNP.g50,dfb1$postNP.g50)
boxplot(dfb1$preNP.g80,dfb1$postNP.g80)

# tests (replicates matlab, although not with same p-values..)
(wilcox.test(dfb1$preNP.g50,dfb1$postNP.g50,paired=TRUE))
(wilcox.test(dfb1$preNP.g80,dfb1$postNP.g80,paired=TRUE))

# for linear models, need to reformat data first
g50_pwr <- c(dfb1$preNP.g50,dfb1$postNP.g50)
g80_pwr <- c(dfb1$preNP.g80,dfb1$postNP.g80)
...
# try slopes as random effects... only improves things for per subject slope for pp
m2 <- lmer(g50_pwr ~ (1|subj) + cueTime + postSpd + xcoord + prevRew,data=npGamma_b1,REML=FALSE,control=lmerControl(optimizer="bobyqa"))
m3 <- lmer(g50_pwr ~ (1|subj) + cueTime + postSpd + xcoord + prevRew + (pp-1|subj),data=npGamma_b1,REML=FALSE,control=lmerControl(optimizer="bobyqa"))
