function fh = PLOT_MotivationalBias_NoSequencesZShuf(cfg_in,data)
% function fh = PLOT_Motivational_NoSequencesZShuf(cfg_in,data)
%
...
for iW = 1:length(what) % loop over epochs
    ...
    this_data = [nanmean(this_data_all{1}) nanmean(this_data_all{2}) nanmean(this_data_all{1})-nanmean(this_data_all{2})];
    ...
    set(gca, 'XTick', -1.5:0.5:1.5, 'FontSize', cfg.fs, 'LineWidth', 1, 'XLim', [-1.5 1.5], 'XTickLabel', {'-1.5 (W)', '', '', 0, '', '', '1.5 (F)'}, 'XDir', 'reverse');
    set(gca, 'YLim' , ylims, 'YTick', 1:3, 'TickDir', 'out', 'YTickLabel', ylab(location));
    title(sprintf('%s', what{iW})); box off
    
    set(gca,'Layer','top')
    
    plot([0 0], ylim, '--', 'Color', [0.5 0.5 0.5]); % vertical line
...
for iRat = 1:length(rats)
    ...
    for iW = 1:length(what)
        ...
        plot([0 0], ylim, '--', 'Color', [0.5 0.5 0.5]);
