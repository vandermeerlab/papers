...
rng(0); % init to reproducible state for shuffles
...
for iCond = 1:nCond
    ...
    %% decode
    cfg_decode = []; cfg_decode.nMinSpikes = cfg.dt; cfg_decode.excludeMethod = 'frate';
    ...
    %% do shuffles, if requested
    if ~isempty(cfg.nShuffles)
       
        shuf_count = expCond(iCond).decode_map; % variable to track, for each bin, how many shuffle sequences spanned it
        shuf_count.data = zeros(size(expCond(iCond).decode_map.data)); 
        
        for iShuf = cfg.nShuffles:-1:1
        
            fprintf('Shuffle %d/%d...\n',iShuf,cfg.nShuffles);
            
            % consistent-identity shuffle
            %shuf_tc = expCond(iCond).tc.tc; shuf_nCells = size(shuf_tc,1);
            %shuf_tc = shuf_tc(randperm(shuf_nCells),:);
            %shuf_P = DecodeZ(cfg_decode,expCond(iCond).Q,shuf_tc); % full decoded probability distribution
            
            % inconsistent-identity shuffle
            shuf_Q = expCond(iCond).Q.data;
            [~,shuf_mask] = sort(rand(size(shuf_Q)),1);
            shuf_Q = shuf_Q(shuf_mask);
            expCond(iCond).Q.data = shuf_Q;
            shuf_P = DecodeZ(cfg_decode,expCond(iCond).Q,expCond(iCond).tc.tc); % NOTE overwrites original Q...
                       
            shuf_P.data(:,expCond(iCond).nNeurons < cfg.nMinNeurons) = NaN; % exclude bins with insufficient neurons
            
            expCond(iCond).seq_iv_shuf{iShuf} = DecSeqDetectZ(cfg_seq,shuf_P);
            
            % update counts
            [~,keep_idx] = restrict(shuf_count,expCond(iCond).seq_iv_shuf{iShuf});
            shuf_count.data(keep_idx) = shuf_count.data(keep_idx) + 1;
    ...
    this_trueZ = tsd(enc_linpos.tvec,expCond(iCond).tc.pos_idx); % true position in units of bins (as established by tuning curves)
