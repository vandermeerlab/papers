...
cfg_master = [];
cfg_master.dt = 0.025;
cfg_master.TCsmooth = 1; % bins SD, choose one
cfg_master.QsmoothSD = 0.01; % time SD, choose one
...
nMaxLaps = 10;
cfg_master.nEncLaps = 1:nMaxLaps; % note, in LOOCV the current lap doesn't get used
cfg_master.encdecmat = 1-eye(nMaxLaps);
...
for iFD = 1:length(fd)
    ...
    for iCond = 1:nCond
        ...
        for iEncLap = 1:length(cfg_master.nEncLaps)
            
            this_Psplice{iEncLap} = tsd;
            this_Psplice{iEncLap}.usr.nActiveNeurons = []; % need to initialize this for UnionTSD() to work
            this_Psplice{iEncLap}.usr.nActiveNeuronsPassed = [];
            
            this_trueZsplice{iEncLap} = tsd;
        ...
        for iLap = 1:nLaps % lap to decode
            ...
            %%
            for iEncLap = 1:length(cfg_master.nEncLaps) % laps to base encoding model on
                
                this_EncLap = cfg_master.nEncLaps(iEncLap);
                fprintf('..EncLap %d/%d...\n',iEncLap,length(cfg_master.nEncLaps));
                ...
                if this_EncLap > nAvailableLaps-1
                    fprintf('EncLap %d not available, skipping...\n',this_EncLap);
                    continue;
                end
                
                % goal: find set of laps that doesn't include current lap
                enc_laps = get_LOOCV_laps([],1:nAvailableLaps,iLap,this_EncLap);
                ...
                % append to tsd
                this_Psplice{iEncLap} = UnionTSD([],this_Psplice{iEncLap},this_lap_P);
                this_trueZsplice{iEncLap} = UnionTSD([],this_trueZsplice{iEncLap},this_trueZ); % could move outside Q-loop
        ...
        for iEncLap = 1:length(cfg_master.nEncLaps)
            ...
            if isempty(this_Psplice{iEncLap}.tvec)
                % no data available, skip
                continue;
            end
            
            [expCond(iCond).Perr,expCond(iCond).confMat] = DecodeErrorZ(cfg_err,this_Psplice{iEncLap},this_trueZsplice{iEncLap});
            ...
            % failed
            goodPbins = logical(this_Psplice{iEncLap}.usr.nActiveNeuronsPassed);
            expCond(iCond).Pnan = sum(isnan(nansum(this_Psplice{iEncLap}.data(:,goodPbins))));
            ...
            ALL_decErr.(expCond(iCond).label).Pnan(iFD,iEncLap) = expCond(iCond).Pnan;
            ALL_decErr.(expCond(iCond).label).Ppassed(iFD,iEncLap) = expCond(iCond).Ppassed;
            
            ALL_decErr.(expCond(iCond).label).meanErr(iFD,iEncLap) = nanmean(expCond(iCond).Perr.data);
            
            ALL_decErr.(expCond(iCond).label).trueZ{iFD} = this_trueZsplice{iEncLap};
            
            cfg_tsdZ = []; cfg_tsdZ.edges = cfg_tc.binEdges{1};
            ALL_decErr.(expCond(iCond).label).spaceErr(iFD,iEncLap,:) = TSDbyZ(cfg_tsdZ,expCond(iCond).linpos,expCond(iCond).Perr);
            
            ALL_decErr.(expCond(iCond).label).nSpikesHist(iFD,iEncLap,:) = histc(this_Psplice{iEncLap}.usr.nActiveNeurons,cfg_master.nSpikesHist);
            
            ALL_decErr.(expCond(iCond).label).confMat{iFD,iEncLap} = expCond(iCond).confMat;
