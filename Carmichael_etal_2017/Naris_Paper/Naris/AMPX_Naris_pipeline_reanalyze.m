function [data_out] = AMPX_Naris_pipeline_reanalyze(fname,data_in, varargin)
...
if ~strcmp(session_type, 'task')
    detect_chan = data_in.lg.cycles.ExpKeys.DetectChan;
end
...
evts.spindles = data_in.spindles.evts;
...
if strcmp(session_type, 'task') ~=1
    ...
    % low gamma
    cfg = []; cfg.freq = [40 55];
    cfg.detect_chan = detect_chan;
    cfg.chan = detect_chan;
...
%% get stats
data_out.stats = AMPX_Naris_get_event_stats(data_out, data_remap_AMPX);
