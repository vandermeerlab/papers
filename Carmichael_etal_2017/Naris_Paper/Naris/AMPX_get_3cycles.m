...
for ievt =1:length(evts_in.tstart);
    ...
    data2.tvec = data_in.tvec(round(i1-buffer):round(i2+buffer));
    for ii = 1:64
        data2.channels{ii} = data_in.channels{ExpKeys.Probe_layout(ii)}(round(i1-buffer):round(i2+buffer));
    end
    clear ii;
    ...
    Remove_ft()
    cfg_filter = [];
    cfg_filter.f = cfg.freq; %filter for band of interest
    cfg_filter.type = 'cheby1';
    cfg_filter.order = 5;
    data_tsd_filt = FilterLFP(cfg_filter, data_tsd);
    
    %% get extract the cycle
    cfg.evt_length = evt_length;
    cfg.chan = cfg.detect_chan;
