...
elseif isfield(data, 'tvec') && isfield(data, 'data')
    type = 'TSD';
elseif isfield(data, 'power_distrib_avg') % this ios a rare case used for testing a remapping script. 
    type = 'POW';
else
...
switch type
    ...
    case 'AMPX'
        ...
        case 'POW'
        disp('This is a power mapping check!!!!! Never use this for anything else')
        pow_avg = reshape(data.power_distrib_avg, 1,64);
        for iChan = 64:-1:1
            data_out.power_distrib_avg(iChan) = pow_avg(ExpKeys.Probe_layout(iChan));
            data_out.labels_remap(iChan) = ExpKeys.Probe_layout(iChan);
            sites_out(iChan) = iChan;
            sites_in(iChan) = ExpKeys.Probe_layout(iChan);
        end
