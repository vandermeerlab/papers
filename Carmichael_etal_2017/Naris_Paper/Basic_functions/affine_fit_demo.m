...
% extra plane
n_3_exact = [-sqrt(2.5)/2 0 sqrt(2.5)/2];
N = 12;
[X,Y] = meshgrid(linspace(0,1,N));
XYZ_3 = [X(:) Y(:) X(:)] + bsxfun(@times,0.05*randn(N^2,1),n_3_exact);
plot3(XYZ_3(:,1),XYZ_3(:,2),XYZ_3(:,3),'m.');
...
% %  norm_data = [mean(XYZ_1); mean(XYZ_1) + (n_1' * (max(max(XYZ_1) - min(XYZ_1))/.5))];
% h = quiver3(p_2(1),p_2(2),p_2(3),norm_data(1),norm_data(2),norm_data(3),'k','linewidth',2)
% h = quiver3(p_2(1),p_2(2),p_2(3),n_2(1)/1,n_2(2)/1,n_2(3)/1,'b','linewidth',2)
plot3([p_2(1) p_2(1)+n_2(1)/1],[p_2(2) p_2(2)+n_2(2)/1], [p_2(3) p_2(3)+n_2(3)/1], 'm')
